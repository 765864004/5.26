...
function getValue(str){
	...
	middle:function(){
		...
		if(sTop<1)sTop="20";
		if(sleft<1)sleft="20";oDialog['style']['left']=sleft+"px";oDialog['style']['top']=sTop+"px";
	},
	reset:function(){
		if(this.config.isReloadOnClose){top.location.reload();};this.close();},close:function(){G('dialogBox').style.display='none';if(!this.config.isBackgroundCanClick)G('dialogBoxBG').style.display='none';if(this.config.isShowShadow)G('dialogBoxShadow').style.display='none';G('dialogBody').innerHTML='';this.showSome();var formObj = document.eDeng;if(typeof(formObj)!='undefined'){if (this.info.form_name && formObj.elements[this.info.form_name]){formObj.elements[this.info.form_name].focus();}}},
	hiddenSome:function(){
		...
		for(var j=0;j<ids.length;j++){this.hiddenEle(ids[j]);};
		space("begin");
	},
...
function closeIt(type,returnurl){
	if(type){
...
function ShowAlert(title,content,rload,w,h,b,f){
	w = w ? w : '400';
	h = h ? h : 'auto';
	b = b ? b : '马上修改';
	f = f ? f : '';
	g_pop=new Popup({contentType:4,isReloadOnClose:rload,width:w,height:h});
	g_pop.setContent("title",title);
	g_pop.setContent("alertCon",content);
	g_pop.setContent("goButton",b);
	g_pop.setContent("form_name",f);
	g_pop.build();
	g_pop.show();
}
